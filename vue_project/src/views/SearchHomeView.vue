<template>
  <div class="home-content">
    <SidePanel :isOpen="showSidePanel" @close="showSidePanel = false" />
    <!-- 顶部固定区域 -->
...
const tallPosts = ref([])
//useRoute()为SearchPage传过来的recomresponse
const route = useRoute()
//把route的内容传给recomresponse
const recomresponse = ref(JSON.parse(route.query.recomresponse || '[]'))

//console.log('recomresponse 更新了：', recomresponse)
//把['1.jpg'，‘2，jpg’]变为postList的格式
const postList = recomresponse.value.map((url, index) => ({
...
const handleImageLoaded = (column, index, actualAspectRatio) => {
  if (column === 'left') {
    ...
    rightColumnHeight.value = rightColumnPosts.value.reduce(
      (sum, post) => sum + post.aspectRatio,
      0
    )
  }
}
