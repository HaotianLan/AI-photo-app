...
class ImageLabelRetriever:
    def __init__(self,
                 ...
                 label_json_path='xhs_school_change2.json',
                 device='mps',
    ...
    def retrieve_similar_images(self, query_image_path):
        ...
        for entry in label_db:
            try:
                ...
                # ✅ 处理提示词 prompt
                prompt_list = entry.get('prompt', [])
                #print(f"[DEBUG] 提取到 prompt: {prompt_list}")

                if isinstance(prompt_list, list):
                    prompt_str = '、'.join(prompt_list)
                elif isinstance(prompt_list, str):
                    prompt_str = prompt_list
                else:
                    prompt_str = ''
                ...
                # ✅ 生成 line_path
                if '/' in img:
                    folder, filename = img.split('/', 1)
                    name_no_ext, _ = os.path.splitext(filename)
                    line_path = f"{folder}_output/{name_no_ext}_edges.png"
                else:
                    name_no_ext, _ = os.path.splitext(img)
                    line_path = f"{name_no_ext}.png"
                ...
                print(f"[ERROR] Failed to process entry: {entry}, reason: {e}")

        # ✅ 排序
        #print('No:{}'.format(scored_images[:10]))
        scored_images.sort(key=lambda x: x.get('score', 0), reverse=True)
        #print('Yes:{}'.format(scored_images[:10]))
        # ✅ 返回格式
        return [
            {
                'img_path': item['img_path'],
                'prompt': item['prompt'],
                'line_path': item['line_path']
            }
